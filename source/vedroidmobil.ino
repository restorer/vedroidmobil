...
// + Прототипы функций, чтоб можно было компилировать вне среды Arduino
void vultrasoundmode();
void vultrasoundmode_old();
void vbluetoothmode();
int vHC_SR04();
void vforward();
void vforwardRL();
...
void setup() {
  ...
  /* Устанавливаем все выводы, к которым подключены светодиоды,
   ...
   для проверки */
  for (int i = 0; i < 6; i++) {
    pinMode(vledpins[i], OUTPUT);
    digitalWrite(vledpins[i], HIGH);
    delay (500);
    digitalWrite(vledpins[i], LOW);
  }
...
void vultrasoundmode() {
  ...
  int distance = vHC_SR04();

  if (distance >= vmindistance) {
    vforward();
    return;
  }
  ...
  for (int max_iterations = 0; max_iterations < 100; max_iterations++) {
    ...
    Serial.print(dist);
    Serial.print(", up to = ");
    Serial.println(vmindistance * 2);

    if (dist > vmindistance * 2) {
      break;
    }
